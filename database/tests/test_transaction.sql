...
-- Assert: Transaction ID should be returned (not NULL)
IF @new_transaction_id IS NULL
    THROW 50101, 'Test 1 FAILED: Expected transaction ID to be returned', 1;

-- Assert: Transaction should be inserted into the table
...
-- Assert: Transaction ID should be returned
IF @new_transaction_id IS NULL
    THROW 50102, 'Test 2 FAILED: Expected transaction ID to be returned', 1;
...
-- Test 3: Multiple transactions should have unique IDs
PRINT 'Test 3: Multiple transactions have unique IDs';
DECLARE @transaction_id_1 UNIQUEIDENTIFIER;
DECLARE @transaction_id_2 UNIQUEIDENTIFIER;
DECLARE @transaction_id_3 UNIQUEIDENTIFIER;

EXEC LogTransaction @user_id = 3, @transaction_type = 1, @resident_id = 1, @new_transaction_id = @transaction_id_1 OUTPUT;
EXEC LogTransaction @user_id = 3, @transaction_type = 1, @resident_id = 1, @new_transaction_id = @transaction_id_2 OUTPUT;
EXEC LogTransaction @user_id = 3, @transaction_type = 1, @resident_id = 1, @new_transaction_id = @transaction_id_3 OUTPUT;
...
EXEC LogTransaction
    ...
    @transaction_type = 1,
    @resident_id = 3,
    @new_transaction_id = @transaction_id_1 OUTPUT;

-- Insert duplicate transaction with same details
EXEC LogTransaction
    @user_id = 5,
    @transaction_type = 1,
    @resident_id = 3,
    @new_transaction_id = @transaction_id_2 OUTPUT;
...
-- Insert first transaction with specific ID
BEGIN TRY
    INSERT INTO Transactions (id, user_id, transaction_type, resident_id)
    VALUES (@duplicate_id, 6, 1, 4);
END TRY
BEGIN CATCH
    THROW 50106, 'Test 6 FAILED: First insert should succeed', 1;
END CATCH

-- Attempt to insert second transaction with same ID (should fail)
BEGIN TRY
    INSERT INTO Transactions (id, user_id, transaction_type, resident_id)
    VALUES (@duplicate_id, 7, 2, 5);
...
BEGIN CATCH
    ...
    ELSE
        THROW 50106, 'Test 6 FAILED: Unexpected error occurred during duplicate insert', 1;
END CATCH
